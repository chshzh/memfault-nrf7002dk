...
static int mqtt_do_connect(void)
{
	...
	struct mqtt_helper_conn_params conn_params = {
		.hostname.ptr = CONFIG_APP_MQTT_CLIENT_BROKER_HOSTNAME,
		.hostname.size = strlen(CONFIG_APP_MQTT_CLIENT_BROKER_HOSTNAME),
...
static void app_mqtt_client_thread(void *arg1, void *arg2, void *arg3)
{
	ARG_UNUSED(arg1);
	ARG_UNUSED(arg2);
	ARG_UNUSED(arg3);
	...
	LOG_INF("App MQTT client thread started");
...
{
	LOG_INF("App MQTT client initialized");
	mqtt_client_running = true;
	return 0;
}

static int app_mqtt_client_module_init(void)
{
	return app_mqtt_client_init();
}
SYS_INIT(app_mqtt_client_module_init, APPLICATION, 2);
